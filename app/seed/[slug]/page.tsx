...
export default async function SeedPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const seed = findBySlug("info", slug);
  if (!seed || seed.type !== "seed") return notFound();

  const branches = loadBranches("info");
  const parent = seed.parent ? branches.find((b) => b.canonical_slug === seed.parent) : undefined;
  const accent = parent ? branchColors[parent.canonical_slug] ?? "var(--accent)" : "var(--accent)";
  const seedHtml = await renderMarkdown(stripHeading(seed.body));
    ...
    <div className="container seed-page">
      <div className="layout">
        ...
        <main>
          <h1>{seed.title}</h1>
          <div className="panel">
            <article className="prose" dangerouslySetInnerHTML={{ __html: seedHtml }} />
          </div>
        </main>
      </div>
    </div>
